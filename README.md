...
| --- | --- | --- |
| `aphelion-core` — fixed-point prices, aggregation math, signing payload | ✅ Implemented | 22 |
| `aphelion-node` — sources, collector, round loop, signer, HTTP API, CLI | ✅ Implemented | 44 |
| `aphelion-registry` contract — identity, stake, reputation, slashing accounting | ✅ Implemented | 20 |
| `aphelion-aggregator` contract — consensus, TWAP, metering | 🚧 In progress | — |
| `aphelion-slashing` contract — disputes, committee voting, appeals | 📋 Planned | — |
| `consumer-example` contract — reference dApp integration | 📋 Planned | — |
| Multi-node integration + Byzantine simulation harness | 📋 Planned | — |
...
├── tests/vectors/              Cross-implementation signing test vectors
...
- Rust 1.91+ (`rustup toolchain install 1.91.0`)
- PostgreSQL 14+
...
```
  consumers ──── metered reads / prepaid balance ────▶ aggregator
                                                           │
                                                    reward pool
                                                           │
...
| Phase | Scope |
| --- | --- |
| **1 — Foundation** *(current)* | Core math and signing payload · node service · registry contract · aggregator contract |
| **2 — Integration** | Slashing contract · consumer-example · multi-node integration harness · testnet deployment |
| **3 — Hardening** | Byzantine simulation suite · dispute governance · Grafana dashboards · external review |
...
# Regenerate the shared signing vectors (must be committed with any layout change)
